...
class Plugin:
    ...
    def on_before_response(self, context):
        ...
        if "face_verify" not in assurance["factors_present"]:
            return self._deny("assurance_missing", reason_details("assurance_missing")["user_message"], risk)
        now = int(time.time())
        if int(assurance["expires_at"]) <= now:
            return self._deny("assurance_expired", reason_details("assurance_expired")["user_message"], risk)
        if assurance["liveness_status"] != "pass":
            return self._deny("liveness_failed", reason_details("liveness_failed")["user_message"], risk)
        return None
    ...
    def _resolve_face_assurance(self, context: dict[str, Any] | None) -> dict[str, Any] | None:
        ...
        runtime = (((self.app_context or {}).get("plugin_manager") or {}).runtimes.get("face_verify")) if (self.app_context or {}).get("plugin_manager") else None
