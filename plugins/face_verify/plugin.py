...
class Plugin:
    ...
    def on_command(self, command, args, context):
        ...
        if command == "/face-verify":
            ...
            if not result.ok:
                details = reason_details(result.reason_code)
                return self._response(
                    command,
    ...
    def _response(self, command: str, content: str, **extra: Any) -> dict[str, Any]:
        payload = {"type": "command_response", "command": command, "content": content}
